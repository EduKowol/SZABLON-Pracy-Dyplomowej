...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}p{0.22\textwidth}X p{0.19\textwidth}@{}}
    ...
    Perl (np. Strawberry Perl) & Interpreter wymagany do uruchomienia programu \texttt{latexmk} & wymagany z \texttt{latexmk} w Windows \\
...
Program \texttt{latexmk}, używany w zalecanym przebiegu kompilacji szablonu, jest skryptem napisanym w języku Perl. W systemie Windows instalacja MiKTeX może zatem nie wystarczyć: potrzebny jest również działający interpreter Perl. Można zainstalować na przykład Strawberry Perl ze strony \url{https://strawberryperl.com/}. Po instalacji należy zamknąć i ponownie otworzyć terminal oraz TeXstudio, aby programy zobaczyły zaktualizowaną zmienną \texttt{PATH}. Sam LuaLaTeX nie wymaga Perla, lecz bez niego nie zadziała opisane w tym szablonie automatyczne budowanie przez \texttt{latexmk}. W Prism, Overleaf i podobnych edytorach internetowych nie instaluje się Perla lokalnie, ponieważ środowiskiem kompilacji zarządza platforma.
...
\begin{description}
  \item[Brak interpretera Perl.] Jeżeli \texttt{latexmk} informuje o braku Perla albo polecenie \texttt{perl -{}-version} nie działa, zainstaluj interpreter (np. Strawberry Perl), a następnie ponownie uruchom terminal i TeXstudio. MiKTeX dostarcza \texttt{latexmk}, ale na Windows interpreter Perla może wymagać osobnej instalacji.
