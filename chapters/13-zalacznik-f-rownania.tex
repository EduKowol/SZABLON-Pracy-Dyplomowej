...
\end{equation}
gdzie pierwszy składnik opisuje błąd dopasowania, a drugi jest składnikiem
regularyzacyjnym.
...
\end{equation}
Warunek umieszcza się po znaku \verb|&|, aby wszystkie opisy były wyrównane.
...
\end{equation}
gdzie przykładowa macierz stanu ma postać
\begin{equation}
  \boldsymbol{A}=
  \begin{bmatrix}
    0 & 1\\
...
\end{equation}
Indeks będący zmienną zapisuje się kursywą, jak w $x_i$, natomiast indeks opisowy
...
\begin{equation}
  ...
  \label{eq:rmse}
\end{equation}
Jeżeli operator występuje wielokrotnie, warto zdefiniować go w pliku stylu poleceniem
...
z dokładności metody, a nie z domyślnych ustawień programu obliczeniowego.
